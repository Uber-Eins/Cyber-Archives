...
function getIP() {
  ...
  if (!v4 && !v6) {
    ...
    if (v4?.primaryAddress) info.push(`设备IPv4：${v4?.primaryAddress}`);
    if (v6?.primaryAddress) info.push(`设备IPv6地址：${v6?.primaryAddress}`);
    if (v4?.primaryRouter && getSSID()) info.push(`路由器IPv4：${v4?.primaryRouter}`);
